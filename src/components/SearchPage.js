...
function SearchPage() {
  ...
  const [queryParams, setQueryParams] = useState({yearMin:2000, yearMax:2002})

  const runSearchQuery = async () => {
    const res = await fetch('https://imdb-api.com/API/AdvancedSearch/k_v3ejgbqw?' + new URLSearchParams(
        {
          title_type: 'feature',
          user_rating: '1.0,2.0',
          count: '100',
